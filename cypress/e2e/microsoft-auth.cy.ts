...
const authBypassEnabled = Cypress.env("authBypass");
...
describe("Microsoft Authentication Flow", () => {
  ...
  if (authBypassEnabled) {
    it("redirects to the dashboard when auth bypass is enabled", () => {
      cy.location("pathname").should("match", /^\/dashboard/);
    });

    it("does not render the Microsoft sign-in button", () => {
      cy.contains("button", "Sign in with Microsoft").should("not.exist");
    });
  } else {
