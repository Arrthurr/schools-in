...
export function generateSrcSet(basePath: string, sizes: number[]): string {
  return sizes.map((size) => `${basePath}?w=${size}&q=75 ${size}w`).join(", ");
}
...
export function generateOptimizedImageURL(
  ...
  if (typeof window === "undefined") return src; // SSR safeguard
...
export function measureImageLoadTime(src: string): Promise<number> {
  return new Promise((resolve, reject) => {
    if (typeof window === "undefined") {
      // Cannot measure in SSR; resolve with 0 to avoid blocking
      return resolve(0);
    }
...
export function createLQIP(
  ...
  return new Promise((resolve, reject) => {
    if (typeof window === "undefined" || typeof document === "undefined") {
      return reject(new Error("LQIP generation not supported during SSR"));
    }
    const img = new Image();
