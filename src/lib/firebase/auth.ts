...
export async function syncUserFromM365(): Promise<M365SyncResult> {
  ...
  try {
    const syncFunction = httpsCallable<M365SyncRequest, M365SyncResult>(
      functions,
      "syncUserFromM365"
    );
    const result = await syncFunction({ email: currentUser.email || undefined });

    console.log(`✅ M365 sync completed:`, result.data);
    console.log(`   Role: ${result.data.role}`);
    console.log(
      `   Assigned locations: ${
        result.data.assignedLocations.map((l) => l.name).join(", ") || "None"
      }`
    );

    if (result.data.removedLocations.length > 0) {
      console.log(
        `   Removed from: ${result.data.removedLocations
          .map((l) => l.name)
          .join(", ")}`
      );
    }
...
/**
 * Wait for the Firestore user document to exist and include a role.
 * Used post-login to ensure downstream consumers read the updated role.
 */
export async function waitForUserDocument(
  ...
  throw new Error("Timed out waiting for user profile to be ready");
}
