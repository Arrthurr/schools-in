...
export { COLLECTIONS, User, Location, Session };
...
export const getCachedCollection = async <T>(
  ...
  const cacheKey = FirebaseCache.generateQueryKey(
    collectionName,
    filters.reduce((acc, f) => ({ ...acc, [`${f.field}_${f.operator}`]: f.value }), {}),
...
export const getCachedUserSessions = async (
  ...
  const cacheKey = `user_sessions_${userId}_${status || 'all'}_${limitCount || 'unlimited'}`;
...
export const batchUpdateWithCache = async (
  ...
  await Promise.all(
    collections.map(collection =>
      FirebaseCache.invalidateCache([`collection_${collection}`, `${collection}_`])
    )
  );
};
...
export const clearFirestoreCache = async (type?: 'users' | 'locations' | 'sessions' | 'assignments') => {
  if (type) {
...
export const preWarmCache = async (userId?: string): Promise<void> => {
  try {
    ...
    await getCachedCollection<Location>(COLLECTIONS.LOCATIONS, { limitCount: 50 });
