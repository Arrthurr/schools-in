...
export class ConfigManager {
  ...
  static getConfig(environment: keyof typeof ConfigManager.configs): EnvironmentConfig {
  ...
  static generateHostingConfig(environment?: string): any {
    const config = environment ? this.getConfig(environment as any) : this.getCurrentConfig();
  ...
  private static generateHeaders(config: EnvironmentConfig): any[] {
    ...
    headers.push({
      ...
      headers: [
        {
          key: 'Cache-Control',
          value: `public,max-age=${config.caching.ttl.static}${config.caching.strategy === 'aggressive' ? ',immutable' : ''}`
        }
      ]
    });
  ...
  static validateConfig(config?: EnvironmentConfig): {
    ...
    if (configToValidate.caching.strategy === 'minimal' && configToValidate.name === 'production') {
  ...
  static getBuildCommand(environment: string): string {
    const commands = {
      ...
      production: 'NODE_ENV=production NEXT_PUBLIC_APP_ENV=production npm run build',
    };

    return commands[environment as keyof typeof commands] || commands.production;
  }
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  console.log('🔧 Current deployment configuration:', ConfigManager.getCurrentConfig());
}
