...
import { getCachedLocationsByProvider, getCachedCollection } from "@/lib/firebase/cachedFirestore";
...
export class CachedSchoolService {
  ...
  static async getAllSchools(
    ...
    const { forceRefresh = false, limit: limitCount, orderBy: orderByOptions } = options;
    
    const cacheKey = FirebaseCache.generateQueryKey(
      'schools_filtered',
      filters,
      orderByOptions ? `${orderByOptions.field}_${orderByOptions.direction}` : 'name',
      limitCount
    );
    ...
    return FirebaseCache.cacheLocationData(
      ...
      async () => {
        ...
        if (filters.providerId) {
          queryRef = query(queryRef, where('assignedProviders', 'array-contains', filters.providerId));
        }

        // Apply status filter
        if (filters.status) {
          queryRef = query(queryRef, where('isActive', '==', filters.status === 'active'));
        }
  ...
  static async searchSchools(
    ...
    const searchKey = `school_search_${searchTerm.toLowerCase()}_${JSON.stringify(filters)}`;

    return FirebaseCache.cacheSearchResults(
      searchKey,
      async () => {
        // Get all schools matching filters first
        const schools = await this.getAllSchools(filters, { forceRefresh: true });
        
        // Perform client-side search
        const searchTermLower = searchTerm.toLowerCase();
        return schools.filter(school => {
          const matchesName = (school.name || '').toLowerCase().includes(searchTermLower);
          const matchesAddress = (school.address || '').toLowerCase().includes(searchTermLower);
          const matchesRegion = (school.region || '').toLowerCase().includes(searchTermLower);
  ...
  static async getSchoolStats(
    ...
    return FirebaseCache.cacheStats(
      ...
      async () => {
        ...
        const activeSchools = allSchools.filter(school => school.isActive !== false).length;
        const schoolsWithProviders = allSchools.filter(school => 
  ...
  static async getNearbySchools(
    ...
    const cacheKey = `nearby_schools_${latitude.toFixed(3)}_${longitude.toFixed(3)}_${radiusKm}_${limitCount}`;
    ...
    return FirebaseCache.cacheLocationData(
      ...
      async () => {
        ...
        const schoolsWithDistance = allSchools
          ...
          .filter((school): school is Location & { distance: number } => 
            school !== null && school.distance <= radiusKm
          )
          .sort((a, b) => a.distance - b.distance);

        return limitCount ? schoolsWithDistance.slice(0, limitCount) : schoolsWithDistance;
      },
      {
        forceRefresh,
        onCacheHit: () => CacheTracker.recordHit(),
  ...
  static async createSchool(schoolData: Omit<Location, 'id'>): Promise<string> {
    const docRef = await addDoc(collection(db, COLLECTIONS.LOCATIONS), schoolData);
