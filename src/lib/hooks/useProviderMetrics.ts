...
interface UseProviderMetricsReturn {
  ...
  startSession: (locationId: string, checkInMethod?: 'geo' | 'manual', distanceFromCenter?: number) => Promise<void>;
...
export function useProviderMetrics(): UseProviderMetricsReturn {
  ...
  const canEndSession = currentSession && ['active', 'paused'].includes(currentSession.status);
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      const activeSession = await CachedSessionService.getActiveSession(user.uid);
      setCurrentSession(activeSession);

      // Fetch weekly sessions (include all sessions, not just completed ones)
      const allWeeklySessions = await CachedSessionService.getUserWeeklySessions(user.uid);
      const completedWeeklySessions = allWeeklySessions.filter(session => session.status === 'completed');
      ...
      // Calculate durations
      const sessionDurations = completedWeeklySessions.map(session => {
        if (session.endTime) {
          return (session.endTime.toDate().getTime() - session.startTime.toDate().getTime()) / (1000 * 60);
        }
        return 0;
      ...
      const locationsVisited = new Set(completedWeeklySessions.map(s => s.locationId)).size;
      ...
      const longestSessionDuration = sessionDurations.length > 0 
        ...
        : 0;
      const shortestSessionDuration = sessionDurations.length > 0 
      ...
      const mostVisitedLocation = Object.keys(locationCounts).length > 0
  ...
  const pauseSession = async () => {
    ...
    try {
      setError(null);
      const updatedSession = await CachedSessionService.pauseSession(currentSession.id);
  ...
  const resumeSession = async () => {
    ...
    try {
      setError(null);
      const updatedSession = await CachedSessionService.resumeSession(currentSession.id);
      setCurrentSession(updatedSession);
    } catch (err) {
