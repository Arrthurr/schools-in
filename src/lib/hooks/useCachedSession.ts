...
import { getCachedUserSessions, getCachedDocument } from '@/lib/firebase/cachedFirestore';
...
export function useCachedSession(userId: string | undefined) {
  ...
  const [realtimeUnsubscribe, setRealtimeUnsubscribe] = useState<Unsubscribe | null>(null);
  ...
  const createSession = useCallback(async (sessionData: Omit<Session, 'id'>) => {
    ...
    try {
      ...
      setState(prev => ({
        ...prev,
        activeSession: newSession.status === 'active' ? newSession : prev.activeSession,
  ...
  const getSessionDuration = useCallback((): number => {
    if (!state.activeSession?.startTime) return 0;
    
    const startTime = state.activeSession.startTime instanceof Date
      ? state.activeSession.startTime
      : state.activeSession.startTime.toDate();
...
export function useAdminSessionMonitoring() {
  ...
  useEffect(() => {
    ...
    const loadActiveSessions = async () => {
      try {
        const { getCachedActiveSessions } = await import('@/lib/firebase/cachedFirestore');
