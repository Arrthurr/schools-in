...
import { ProductionMonitoring, ANALYTICS_EVENTS, PERFORMANCE_METRICS } from '../firebase/productionConfig';
...
export function useAnalytics(options: AnalyticsOptions = {}) {
  ...
  const trackEvent = useCallback((
    ...
    if (process.env.NODE_ENV === 'production') {
      ProductionMonitoring.trackEvent(eventName, {
        ...parameters,
        user_role: user?.role,
      }, userProperties);
    }
  ...
  return {
    ...
    trackLogin: (method: string) => trackEvent(ANALYTICS_EVENTS.LOGIN, { method }),
    trackLogout: () => trackEvent(ANALYTICS_EVENTS.LOGOUT),
    trackCheckIn: (locationData: any) => trackLocationEvent('check_in', locationData),
    trackCheckOut: (locationData: any) => trackLocationEvent('check_out', locationData),
    // Performance tracking shortcuts
    trackCheckInDuration: (duration: number) => trackPerformance(PERFORMANCE_METRICS.CHECK_IN_DURATION, duration),
    trackLocationLoadTime: (duration: number) => trackPerformance(PERFORMANCE_METRICS.LOCATION_LOAD_TIME, duration),
    trackSessionListLoad: (duration: number) => trackPerformance(PERFORMANCE_METRICS.SESSION_LIST_LOAD, duration),
    trackUserSearchTime: (duration: number) => trackPerformance(PERFORMANCE_METRICS.USER_SEARCH_TIME, duration),
    trackCacheResponseTime: (duration: number) => trackPerformance(PERFORMANCE_METRICS.CACHE_RESPONSE_TIME, duration),
  };
}
