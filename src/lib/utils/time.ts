...
export function timestampToDateInTimezone(timestamp: Timestamp): Date {
  return new Date(timestamp.toDate().toLocaleString('en-US', { timeZone: APP_TIMEZONE }));
}
 ...
 * Returns timestamps that can be used for Firestore queries
 */
export function getTodayRangeForDate(date: Date): { start: Timestamp; end: Timestamp } {
  const timezoneDate = convertToTimezone(date);
 ...
 * Returns timestamps that can be used for Firestore queries
 */
export function getLastNDaysRange(days: number): { start: Timestamp; end: Timestamp } {
  const today = getCurrentDateInTimezone();
 ...
 */
export function formatTimestampForDisplay(timestamp: Timestamp, options?: Intl.DateTimeFormatOptions): string {
  ...
  return timestamp.toDate().toLocaleString('en-US', { ...defaultOptions, ...options });
}
...
export function calculateDurationMinutes(
  ...
  if (pausedIntervals) {
    for (const interval of pausedIntervals) {
      const pausedMinutes = (interval.end.toMillis() - interval.start.toMillis()) / (1000 * 60);
...
export function isThisWeek(date: Date): boolean {
  ...
  return timestamp.toMillis() >= start.toMillis() && timestamp.toMillis() <= end.toMillis();
}