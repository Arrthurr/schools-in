...
export async function GET(request: NextRequest) {
  try {
    ...
    if (providerId) {
      ...
      querySnapshot = await getSchoolsByProvider(providerId);
    } else {
      // Get all schools (admin view)
      querySnapshot = await getCollection(COLLECTIONS.SCHOOLS);
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const schoolId = await createDocument(COLLECTIONS.SCHOOLS, schoolData);
...
export async function PUT(request: NextRequest) {
  try {
    const body = await request.json();
    const { schoolId, name, address, coordinates, radius, assignedProviders, isActive } = body;
    ...
    await updateDocument(COLLECTIONS.SCHOOLS, schoolId, updateData);

    const updatedSchool = await getDocument(COLLECTIONS.SCHOOLS, schoolId);
